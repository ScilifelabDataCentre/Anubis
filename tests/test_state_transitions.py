...
def review_assignment(settings, browser, transition_call):
    ...
    # Teardown: delete review then proposal
    ...
    page.goto(f"{base}/proposal/{transition_call}:001")
    delete_btn = page.get_by_role("button", name="Delete")
    if delete_btn.is_visible():
        page.once("dialog", lambda d: d.accept())
        delete_btn.click()
...
def test_missing_proposal_and_decision_prereqs():
    pass

def test_deletion_of_call_with_proposals():
    pass